"""日期时间工具集：时间计算、格式转换、工作日等。"""

import calendar
from datetime import datetime, timedelta, timezone
from typing import Any

from app.services.llm.tools.decorator import tool


# ============================================================
# 当前时间
# ============================================================

@tool(
    category="datetime",
    description="获取当前日期和时间（含星期几）",
    triggers=["现在几点", "当前时间", "今天日期", "今天星期几", "现在时间"],
    trigger_patterns=[r"(现在|当前|今天).*(几点|时间|日期|星期)"],
    trigger_priority=10,
    examples=["[TOOL_CALL: current_time(timezone_offset=8)]"],
    pre_execute_pattern=r"(现在|当前|今天).*(几点|时间|日期|星期)",
    pre_execute_extractor=lambda m: {"timezone_offset": 8},
    pre_execute_formatter=lambda r: f"当前时间: {r.get('datetime')} ({r.get('weekday')})",
)
async def current_time(timezone_offset: int = 8) -> dict[str, Any]:
    """
    获取当前时间（默认东八区）。

    Args:
        timezone_offset: 时区偏移(小时)，默认8(北京)
    """
    tz = timezone(timedelta(hours=timezone_offset))
    now = datetime.now(tz)
    return {
        "datetime": now.strftime("%Y-%m-%d %H:%M:%S"),
        "date": now.strftime("%Y-%m-%d"),
        "time": now.strftime("%H:%M:%S"),
        "weekday": ["周一", "周二", "周三", "周四", "周五", "周六", "周日"][now.weekday()],
        "timestamp": int(now.timestamp()),
        "timezone": f"UTC+{timezone_offset}",
    }


# ============================================================
# 日期计算
# ============================================================

@tool(
    category="datetime",
    description="计算两个日期之间相隔多少天/周/月",
    triggers=["相隔多少天", "间隔多少天", "距离多少天", "相差几天"],
    trigger_patterns=[r"\d{4}[-/]\d{1,2}[-/]\d{1,2}.*\d{4}[-/]\d{1,2}[-/]\d{1,2}"],
    trigger_priority=9,
    examples=["[TOOL_CALL: date_difference(date1='2024-01-01', date2='2024-12-31')]"],
    pre_execute_pattern=r"(\d{4}[-/]\d{1,2}[-/]\d{1,2}).*?(\d{4}[-/]\d{1,2}[-/]\d{1,2})",
    pre_execute_extractor=lambda m: {"date1": m.group(1).replace("/", "-"), "date2": m.group(2).replace("/", "-")},
    pre_execute_formatter=lambda r: f"{r.get('date1')} 到 {r.get('date2')} 相隔 {r.get('days')} 天（约 {r.get('weeks')} 周）",
)
async def date_difference(date1: str, date2: str) -> dict[str, Any]:
    """
    计算两个日期之间的天数差。格式: YYYY-MM-DD。

    Args:
        date1: 日期1 (YYYY-MM-DD)
        date2: 日期2 (YYYY-MM-DD)
    """
    try:
        d1 = datetime.strptime(date1, "%Y-%m-%d")
        d2 = datetime.strptime(date2, "%Y-%m-%d")
        diff = abs((d2 - d1).days)
        return {
            "date1": date1, "date2": date2,
            "days": diff,
            "weeks": round(diff / 7, 1),
            "months_approx": round(diff / 30.44, 1),
        }
    except ValueError as e:
        return {"error": f"日期格式错误: {e}"}


@tool(
    category="datetime",
    description="在指定日期上加减天数，得到新日期",
    triggers=["天之后", "天以后", "天后是", "天前是"],
    trigger_patterns=[r"\d+天(之?后|以后|前)"],
    trigger_priority=8,
    examples=["[TOOL_CALL: add_days(date='2024-03-01', days=90)]"],
)
async def add_days(date: str, days: int) -> dict[str, Any]:
    """
    在日期上加/减天数。

    Args:
        date: 起始日期 (YYYY-MM-DD)
        days: 加减天数(负数为减)
    """
    try:
        d = datetime.strptime(date, "%Y-%m-%d")
        result = d + timedelta(days=days)
        return {
            "original_date": date,
            "days_added": days,
            "result_date": result.strftime("%Y-%m-%d"),
            "weekday": ["周一", "周二", "周三", "周四", "周五", "周六", "周日"][result.weekday()],
        }
    except ValueError as e:
        return {"error": f"日期格式错误: {e}"}


@tool(
    category="datetime",
    description="判断指定日期是否为工作日",
    triggers=["工作日", "上班", "休息日", "周末"],
    trigger_patterns=[r"\d{4}[-/]\d{1,2}[-/]\d{1,2}.*(工作日|上班|休息)"],
    trigger_priority=7,
    examples=["[TOOL_CALL: is_workday(date='2024-10-01')]"],
)
async def is_workday(date: str) -> dict[str, Any]:
    """
    判断日期是否为工作日（不含法定假日）。

    Args:
        date: 日期 (YYYY-MM-DD)
    """
    try:
        d = datetime.strptime(date, "%Y-%m-%d")
        is_work = d.weekday() < 5
        return {
            "date": date,
            "weekday": ["周一", "周二", "周三", "周四", "周五", "周六", "周日"][d.weekday()],
            "is_workday": is_work,
        }
    except ValueError as e:
        return {"error": f"日期格式错误: {e}"}


@tool(
    category="datetime",
    description="计算两个日期之间有多少个工作日",
    triggers=["几个工作日", "多少个工作日"],
    trigger_priority=7,
    examples=["[TOOL_CALL: workdays_between(date1='2024-01-01', date2='2024-01-31')]"],
)
async def workdays_between(date1: str, date2: str) -> dict[str, Any]:
    """
    计算两个日期之间的工作日数量。

    Args:
        date1: 起始日期 (YYYY-MM-DD)
        date2: 结束日期 (YYYY-MM-DD)
    """
    try:
        d1 = datetime.strptime(date1, "%Y-%m-%d")
        d2 = datetime.strptime(date2, "%Y-%m-%d")
        if d1 > d2:
            d1, d2 = d2, d1
        count = 0
        current = d1
        while current <= d2:
            if current.weekday() < 5:
                count += 1
            current += timedelta(days=1)
        return {"date1": date1, "date2": date2, "workdays": count}
    except ValueError as e:
        return {"error": f"日期格式错误: {e}"}


@tool(
    category="datetime",
    description="将Unix时间戳转换为可读的日期时间",
    triggers=["时间戳", "timestamp"],
    trigger_patterns=[r"1[0-9]{9}"],
    trigger_priority=8,
    examples=["[TOOL_CALL: timestamp_convert(timestamp=1700000000)]"],
)
async def timestamp_convert(timestamp: int) -> dict[str, Any]:
    """
    Unix时间戳转可读日期时间。

    Args:
        timestamp: Unix时间戳(秒)
    """
    tz = timezone(timedelta(hours=8))
    dt = datetime.fromtimestamp(timestamp, tz)
    return {
        "timestamp": timestamp,
        "datetime": dt.strftime("%Y-%m-%d %H:%M:%S"),
        "date": dt.strftime("%Y-%m-%d"),
        "timezone": "UTC+8",
    }


@tool(
    category="datetime",
    description="获取指定月份的日历信息（天数、起始星期等）",
    triggers=["日历", "月份天数", "几天"],
    trigger_priority=5,
    examples=["[TOOL_CALL: month_calendar(year=2024, month=2)]"],
)
async def month_calendar(year: int, month: int) -> dict[str, Any]:
    """
    获取指定月份的日历信息。

    Args:
        year: 年份
        month: 月份(1-12)
    """
    try:
        cal = calendar.monthcalendar(year, month)
        _, days_in_month = calendar.monthrange(year, month)
        first_weekday = calendar.weekday(year, month, 1)
        return {
            "year": year, "month": month,
            "days_in_month": days_in_month,
            "first_day_weekday": ["周一", "周二", "周三", "周四", "周五", "周六", "周日"][first_weekday],
            "weeks": len(cal),
        }
    except Exception as e:
        return {"error": str(e)}
